$(document).ready(function () {
    function fetchCanceledOrderData(filter = 'day') {
        const requestData = { filter: filter };

        $.ajax({
            url: '{{ route("dashboard.orders.canceled") }}', // Sử dụng URL từ route đã đặt tên
            type: 'GET',
            data: requestData,
            success: function (response) {
                ...
                $('#customers-filter-title-3').text(`| ${filter === 'day' ? 'Hôm nay' : filter === 'month' ? 'Tháng này' : 'Năm nay'}`);
                $('#customers-count-3').text(ordersCurrent.toLocaleString('vi-VN'));
                $('#customers-increase-3').text(`${Math.abs(percentageChange).toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 1 })}%`).removeClass('text-success text-danger').addClass(changeClass);
                $('#customers-increase-3').next('.text-muted').text(changeText);
            },
