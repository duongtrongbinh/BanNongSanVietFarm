$(document).ready(function () {
    function fetchShippingOrderData(filter = 'day') {
        ...
        $.ajax({
            url: '/api/orders/shipping', // Sử dụng URL từ route đã đặt tên
            type: 'GET',
            data: requestData,
            success: function (response) {
                ...
                $('#customers-filter-title-1').text(`| ${filter === 'day' ? 'Hôm nay' : filter === 'month' ? 'Tháng này' : 'Năm nay'}`);
                $('#customers-count-1').text(ordersCurrent.toLocaleString('vi-VN'));
                $('#customers-increase-1').text(`${Math.abs(percentageChange).toLocaleString('vi-VN', {
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 1
                })}%`).removeClass('text-success text-danger').addClass(changeClass);
                $('#customers-increase-1').next('.text-muted').text(changeText);
            },
