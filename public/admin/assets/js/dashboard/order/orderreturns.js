$(document).ready(function () {
    function fetchRefundOrderData(filter = 'day') {
        const requestData = { filter: filter };

        $.ajax({
            url: '/api/orders/Returns', // Sử dụng URL từ route đã đặt tên
            type: 'GET',
            data: requestData,
            success: function (response) {
                ...
                $('#refund-filter-title').text(`| ${filter === 'day' ? 'Hôm nay' : filter === 'month' ? 'Tháng này' : 'Năm nay'}`);
                $('#refund-count').text(ordersCurrent.toLocaleString('vi-VN'));
                $('#refund-increase').text(`${Math.abs(percentageChange).toLocaleString('vi-VN', { minimumFractionDigits: 0, maximumFractionDigits: 1 })}%`).removeClass('text-success text-danger').addClass(changeClass);
                $('#refund-increase').next('.text-muted').text(changeText);
            },
